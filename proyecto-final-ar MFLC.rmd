...
```{r}
NOMBRE_COMPLETO = "María Fernanda López Cruz"
DIGITOS_MATRICULA = 01745765
```
...
```{r}
numTransactions <- round(length(smallbasket))
summary(smallbasket)
```
...
- Cantidad de reglas producidas (**NOTE-16**):5
- Media aritmética de support (**NOTE-17**): 0.2478
- Media aritmética de confidence (**NOTE-18**): 1
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**): Que existe la regla en la que solo se consumen diapers sin consumir ningun otro producto de la lista
- Calcula el valor de la métrica support para la regla `{beer} => {nuts}` (**NOTE-20**): 0.2
- Calcula el valor de la métrica confidence para la regla `{beer} => {nuts}` (**NOTE-21**): 1
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Porque tiene un confidence de 1 y el maximo especificado fue 0.8
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Darle un id unico a la sesión de escucha de cierto track
- ¿Cuál es el propósito de `track id`? (**NOTE-24**):Darle un id unico a cada cancion dentro de la plataforma
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**):Yo tambien ocuparía la columna "Not Skipped" para asi poder asocial de manera veridica un comportamiento veridico del usuario con ese track y conjuntarlo con algun otro track 
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): 0_098493b5-ae01-4412-afbc-5b703087a27d con 6502 veces

- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): 5
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): 14.62  Que la media de items incluidos son 14.62 
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**): Que la frecuencia de ciertos items no tienen un confidence por encima de lo especificado, es decir son poco reproducidos y no producen ninguna regla de asociacion.
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? La creacion de demasiadas reglas, la satauracion y entorpecimiento del analisis enfocado a un comportamiento repetitivo 
...
- Provee el enlace al video aquí: **NOTE-30** https://youtu.be/r1IdHP8vRnU
